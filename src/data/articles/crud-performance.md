...
Assumption: "Native code is _faster_ than Interpreted code", But is it faster in the things that matter?

My test case for latency is to benchmark 6 languages with a simple REST API, for more details about the implementation have a look at [REST API and in memory implementation](crud-openapi).
...
Assumption: First requests should be slower, as we are interpreting code and using CPU time to compile code. 
...
So I decided to run a benchmark on an environment that I know well and love. Serverless container in Google Cloud Run. I can deploy docker containers, you pay per request usage, it scales up and down automatically, it has a CPU boost at startup to avoid cold start.

I ran a simple scenario that does a series of CRUD operations in memory, it will tell me how fast each language is to respond to and handle HTTP requests.
...
I was happily surprised that even for this simple load, everything was able to respond under 10ms 95% of the time. And 50% of the requests have a latency of 5ms or less, for all stacks. Yes, there are some flukes (probably JIT compiling) in some languages that I'm sure we can optimize or get around by tuning the specific ones. But my general first thought is that no matter if you are using Go or Python, the bottleneck is not in the pure language.

**DEBUNKED**: No matter if it is native code or byte code, if using [^1]JIT or [^2]AOT, everything runs under 10ms.
...
Is this any indicator of where the bottleneck will be? Maybe yes, maybe it's just how the different implementations of the in-memory storage work. Let's continue our investigation, this alone isn't enough.
...
Same machine in Cloud Run for this run, with a Postgres Cloud SQL instance (2 CPU, 8 GB RAM), that should be plenty to not be the bottleneck.
...
1. Go and C# handle 400 requests per second with a response time of 10ms or less 95% of the time.
2. TypeScript and Java handle 300 requests per second with a response time of 10ms or less 95% of the time.
3. Kotlin handles 250 requests per second with a response time of 12ms or less 95% of the time.
...
Except Python and Kotlin, everything else had similar response time around 10ms, around 1-2ms slower than the in-memory version. It seems that our database was overkill with 2 CPUs and 8GB of RAM, even if preloaded with 10,000 rows, it acted more like a memory store than giving db perf, but it's hard to isolate what you are testing.


So did our assumptions hold true?

1) Go is first without a sweat, as expected — fast and good at parallelism. But C# .NET 10 being on par was a surprise.
2) TypeScript and Java are a bit of a surprise here:
	- Java is using JDBC for database connections, meaning it blocks the thread where the request is running.
	- TypeScript is using a single-threaded event loop, but the I/O operations give back time to the scheduler, making it more efficient than expected.
3) Kotlin — I was not expecting it to be here, at the same level or better than Java.
4) Python — while I was expecting it to be last, not by that margin.

**DEBUNKED**: At least for non-Go results. Single-threaded is as fast as multi-threaded, at least on containers with 1 CPU. And blocking I/O can be as fast as non-blocking I/O. 🤯
...
Could using another TypeScript runtime like [Node.js 24, Deno or Bun](https://betterstack.com/community/guides/scaling-nodejs/nodejs-vs-deno-vs-bun/) bring it even closer to Go? Probably!
Could [^2]AOT compilation for [C#](https://learn.microsoft.com/en-us/dotnet/core/deploying/native-aot/?tabs=linux-alpine%2Cnet8), [Java (GraalVM)](https://docs.spring.io/spring-boot/maven-plugin/aot.html) or [Kotlin Native](https://kotlinlang.org/docs/native-overview.html) reduce the cold start? Definitely!
...
- Would a multi-CPU container have changed the results?
